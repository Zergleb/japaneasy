var parse = function (text) {
    ...
    if (pre) {
        ...
        return ["No items were found; try another query."];
    }
    return arr;
    function glossingEntries() {
        ...
        while (li != null) {
            ...
            if (pn)
                arr[i].pronunciation = pn[0].substring(1, pn[0].length - 1);
            ...
            if (en) {
                ...
                if (ps) {
                    arr[i].english[0] = stor[i].substring(ps["index"] + ps[0].length).trim().replace("\/", "").replace(/\<\/li\>/, "");
                }
                else {
                    arr[i].english[0] = stor[i].replace('\/', "").replace(/\<\/li\>/, "");
                }
            }
        ...
        for (var i = 0; i < parsed.length - 2; i++) {
            ...
            if (en) {
                ...
                if (ps) {
                    ...
                    arr[i].english[0] = parsed[i + 1].replace('\/', "");
                }
            }
