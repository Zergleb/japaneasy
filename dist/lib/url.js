var url = function (config) {
    ...
    if (config.dictionary) {
        if (typeof config.dictionary != "string")
            throw new Error("Dictionary input must be a string");
        if (!mapDictionary(config.dictionary))
            throw new Error("Invalid parameter for dictionary");
    ...
    if (config.method) {
        if (typeof config.method != "string")
            throw new Error("Method input must be a string");
        if (!mapMethod(config.method))
            throw new Error("Invalid parameter for method");
    ...
    if (config.encoding) {
        if (typeof config.encoding != "string")
            throw new Error("Encoding input must be a string");
        if (!encode(config.encoding))
            throw new Error("Invalid parameter for encoding");
    }
    if (config.custom) {
        if (typeof config.custom != "string")
            throw new Error("Custom input must be a string");
        if (config.custom.length < 4)
            throw new Error("Custom input must be at least 4 characters long");
        custom = config.custom;
    }
    if (config.input == "japanese") {
        last = "J";
    }
    else if (config.input == "english") {
        last = "E";
    }
    else {
        last = "P";
    }
    if (config.dictionary == "glossing" && config.noRepeat) {
        last = "G";
    }
    if (custom)
        return custom;
